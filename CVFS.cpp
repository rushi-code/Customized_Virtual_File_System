...
void man(char *name)
{
    ...
    {
        printf("Description : Used to display all files\n");
        printf("Usage : ls\n");
    }
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    if (read_size < isize)
    {
        ...
        arr[read_size] = '\0'; // Null-terminate the string
    ...
    else
    {
        ...
        arr[isize] = '\0'; // Null-terminate the string
        return isize; // Return the actual number of bytes read
    }
}
...
int stat_file(char * name)
{
    ...
    printf("File Permsission : Read & Write \n");
    printf("----------------------------------------------------------\n");
...
int main()
{
    ...
    while(1)
    {
        ...
        {
            if(strcmp(command [0], "stat")==0)  //stat Rushi.txt
            {
            ...
            else if(strcmp(command [0],"fstat")==0) //fstat Rushi.txt
            {
                ...
                // ret = fstat_file(GetFDFromName(command[1]));
                // if(ret == -1)
                // printf("ERROR : Incorrect parametres \n");
                // if(ret == -2)
                // printf("ERROR : There is no such file \n");
            ...
            else if(strcmp(command[0],"close")==0) //close Rushi.txt
            {
            ...
            else if(strcmp(command[0], "rm") ==0) //rm Rushi.txt
            {
            ...
            else if(strcmp(command[0],"man")==0) //man create/read/open/lseek....
            {
                man(command[1]);
            }
            else if (strcmp(command[0], "write") == 0) // write Rushi.txt
            {
                ...
                // Read input using fgets to handle whitespace-only input
                if (fgets(arr, MAXFILESIZE, stdin) == NULL)
                {
                ...
                if (strlen(arr) == 0 || strspn(arr, " \t\n\r") == strlen(arr))
                {
                    arr[0] = ' '; // Write a single space character
                    arr[1] = '\0'; // Null-terminate the string
                }

                // Calculate input size
                int size = strlen(arr);

                // Call WriteFile with the validated input
                ret = WriteFile(fd, arr, size);
                if (ret == -1)
                    printf("ERROR: Permission denied \n");
                else if (ret == -2)
                    printf("ERROR: There is not sufficient memory to write \n");
                else if (ret == -3)
                    printf("ERROR: It is not a regular file \n");
            }
            else if(strcmp(command[0],"truncate")==0) //truncate Rushi.txt
            {
        ...
        {
            if(strcmp(command[0],"create")==0) //create Rushi.txt 3
            {
            ...
            else if (strcmp(command[0],"open")==0) //open Rushi.txt 
            {
            ...
            else if (strcmp(command[0], "read") == 0) //read Rushi.txt 10
            {
                fd = GetFDFromName(command[1]);
                if (fd == -1)
                {
                ...
                free(ptr); // Free the allocated memory to avoid memory leak
                continue;
            }
        ...
        {
            if(strcmp(command[0],"lseek")==0) // lseek Rushi.txt 0 0 // lseek Rushi.txt -32  2 // lseek Rushi.txt 10 1
            {
                fd = GetFDFromName(command[1]);
                printf("fd is %d\n",fd);
                if(fd == -1)
